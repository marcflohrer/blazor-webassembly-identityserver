...
public class BffAuthenticationStateProvider : AuthenticationStateProvider
{
    ...
    public override async Task<AuthenticationState> GetAuthenticationStateAsync()
    {
        ...
        if (user?.Identity?.IsAuthenticated ?? throw new ArgumentException("user identity is null"))
        {
            ...
            timer = new Timer(async _ =>
            {
                ...
                if (!currentUser?.Identity?.IsAuthenticated ?? throw new ArgumentException("user identity is null"))
                {
                    _logger.LogInformation("user logged out");
                    NotifyAuthenticationStateChanged(Task.FromResult(new AuthenticationState(currentUser ?? throw new ArgumentException("user identity is null"))));
